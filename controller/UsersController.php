...
class UsersController extends BaseController {
	...
	*/
	public function login() {
		...
		if (isset($_POST["dni"])){ // reaching via HTTP Post...
			//process login form
			if ($this->userMapper->isValidUser($_POST["dni"], $_POST["passwd"])) {

				$_SESSION["currentuser"]=$_POST["dni"];
				$_SESSION["currentusername"]=$this->userMapper->getUserNameById($_POST["dni"]);
				...
				$errors = array();
				$errors["general"] = "DNI is not valid";
	...
	*/
	public function register() {
		...
		if (isset($_POST["dni"])){ // reaching via HTTP Post...
			...
			$user->setNombreCompleto($_POST["nombre"]);
			$user->setEmail($_POST["email"]);

			$deportista->setDni($_POST["dni"]);
			$deportista->setGenero($_POST["genero"]);

			try{
				$user->checkIsValidForRegister(); // if it fails, ValidationException

				// check if user exists in the database
				if (!$this->userMapper->usernameExists($_POST["dni"])){

					// save the User object into the database
					$this->userMapper->save($user);
					$this->athleteMapper->save($deportista);
					...
					$this->view->setFlash("Username ".$user->getDni()." successfully added. Please login now");
					...
					$errors = array();
					$errors["dni"] = "Username already exists";
					$this->view->setVariable("errors", $errors);
				}
	...
	*/
	...
	public function messages() {
		$messages = $this->userMapper->getMessages($_SESSION["currentuser"]);

		if($messages == null){
			$messages = [];
		}

		$this->view->setVariable("notifications", $messages);
		// render the view (/view/users/login.php)
